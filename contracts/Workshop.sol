...
contract WorkshopChain {
     ...
     */
    function addParticipant(string memory _name, string memory _email) external {
    ...
    */
    function getParticipants(uint _workshopIndex) external view returns(Participant[] memory){
    ...
    */
    function getCurrentWorkshops() external view returns(Workshop[] memory){
    ...
    */
    ...
    function registerForWorkshop(uint _workshopIndex)external{
        //check values

        Workshop memory w1=workshops[_workshopIndex];
        uint pLen=w1.participants.length;
        workshops[_workshopIndex].participants[pLen]=(participants[msg.sender]);
    }
